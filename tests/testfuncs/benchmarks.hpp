...
template <class T>
class Cluster2D1Benchmark : public Benchmark<T> {
public:

    virtual T calcFunc(const std::vector<T>& point) const {
        return ::calcFunc(Cluster2D1<T>(), point);
    }

    virtual Interval<T> calcInterval(const std::vector<Interval<T>>&box) const {
        return ::calcInterval(Cluster2D1<Interval < T >> (), box);
    }

    virtual ValDer<T> calcGrad(const std::vector<T>& point) const {
        return ::calcGrad(Cluster2D1<ValDer < T >> (), point);
    }

    virtual IntervalDer<T> calcIntervalGrad(const std::vector<Interval<T>>&box) const {
        return ::calcIntervalGrad(Cluster2D1<IntervalDer < T >> (), box);
    }

    Cluster2D1Benchmark() : Benchmark<T>("Cluster2D1 function", -1) {
            this->mGlobMinX.push_back(0);
            this->mGlobMinX.push_back(1);
            this->mBounds.push_back({0, 1});
            this->mBounds.push_back({0, 1});
...
template <class T>
class Cluster2D2Benchmark : public Benchmark<T> {
public:

    virtual T calcFunc(const std::vector<T>& point) const {
        return ::calcFunc(Cluster2D2<T>(), point);
    }

    virtual Interval<T> calcInterval(const std::vector<Interval<T>>&box) const {
        return ::calcInterval(Cluster2D2<Interval < T >> (), box);
    }

    virtual ValDer<T> calcGrad(const std::vector<T>& point) const {
        return ::calcGrad(Cluster2D2<ValDer < T >> (), point);
    }

    virtual IntervalDer<T> calcIntervalGrad(const std::vector<Interval<T>>&box) const {
        return ::calcIntervalGrad(Cluster2D2<IntervalDer < T >> (), box);
    }

    Cluster2D2Benchmark() : Benchmark<T>("Cluster2D2 function", -1) {
            this->mGlobMinX.push_back(0);
            this->mGlobMinX.push_back(0);
            this->mGlobMinX.push_back(0);
            this->mGlobMinX.push_back(1);
            this->mBounds.push_back({0, 0.3});
            this->mBounds.push_back({0, 0.2});
            this->mBounds.push_back({0.7, 1.});
            this->mBounds.push_back({0.8, 1.});
    }
};
