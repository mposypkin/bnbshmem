...
std::atomic<int> gNumRunningThreads(0);

std::vector<BnBStat> stat;

std::mutex gRecordLock;

std::mutex gThreadRunLock;

std::condition_variable gThreadRunCV;
...
void solve(std::shared_ptr<State> s, const BM& bm) {
    ...
    while (!s->mPool.empty()) {
        steps++;
        if (s->mPool.size() >= 2) {
            if (gNumRunningThreads < gProcs) {
                std::lock_guard<std::mutex> lock(gThreadRunLock);
    ...
    gSteps += steps;
    std::unique_lock<std::mutex> lock(gThreadRunLock);
    gNumRunningThreads--;
    gThreadRunCV.notify_one();
}
...
#else    
    gNumRunningThreads = 1;
    solve(s, bm);
    {
        std::unique_lock<std::mutex> lock(gThreadRunLock);
        gThreadRunCV.wait(lock, []() {
            return gNumRunningThreads == 0;
        });
        std::cout << "==" << gNumRunningThreads << "==\n";
    }
#endif
